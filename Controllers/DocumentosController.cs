...
namespace ContpaqiBridge.Controllers
{
    ...
    public class DocumentosController : ControllerBase
    {
        ...
        public IActionResult CrearFactura([FromBody] FacturaRequest request)
        {
            try
            {
                ...
                var productos = (request.Productos ?? new List<ProductoFactura>())
                    .Select(p => (p.Codigo, p.Cantidad, p.Precio, p.Nombre ?? "", p.UnidadMedida ?? "", p.ClaveSAT ?? ""))
                    .ToList();
        ...
        /// <summary>
        /// Obtiene el XML (o PDF) de una factura timbrada. MEJORA #10: formato 0=XML, 1=PDF.
        /// </summary>
        [HttpGet("xml")]
        public IActionResult ObtenerXml([FromQuery] string rutaEmpresa, [FromQuery] string codigoConcepto, [FromQuery] string serie, [FromQuery] double folio, [FromQuery] int formato = 0)
        {
            try
            {
                _logger.LogInformation($"Solicitud de archivo: Concepto={codigoConcepto}, Serie={serie}, Folio={folio}, Formato={formato}");
                var resultado = _sdkService.ObtenerXml(rutaEmpresa, codigoConcepto, serie ?? "", folio, formato);
                
                if (resultado.exito)
                {
                    return Ok(new { success = true, archivo = resultado.xml, formato = formato == 1 ? "pdf" : "xml" });
                }
                else
                {
                    return BadRequest(new { success = false, message = resultado.mensaje });
            ...
            {
                _logger.LogError(ex, "Error en endpoint de archivo");
                return StatusCode(500, new { success = false, message = ex.Message });
            }
        }

        /// <summary>
        /// Valida un documento antes de timbrarlo para detectar errores críticos
        /// (sin enviar al PAC/SAT).
        /// </summary>
        [HttpPost("validar")]
        public IActionResult Validar([FromBody] TimbrarRequest request)
        {
            try
            {
                if (string.IsNullOrEmpty(request.RutaEmpresa))
                ...
                _logger.LogInformation($"Validando pre-timbrado: {request.Serie}{request.Folio}");

                var issues = _sdkService.ValidarParaTimbrado(
                    request.RutaEmpresa,
                    request.CodigoConcepto,
                    request.Serie ?? "",
                    request.Folio,
                ...
                int errores = issues.Count(i => i.severidad == "Error");
                int warnings = issues.Count(i => i.severidad == "Warning");
                bool listoParaTimbrar = errores == 0;
                ...
                return Ok(new
                {
                    ...
                    errores,
                    warnings,
                    issues = issues.Select(i => new { severidad = i.severidad, categoria = i.categoria, mensaje = i.mensaje })
                });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error al validar");
                return StatusCode(500, new { success = false, message = ex.Message });
            }
        }
        ...
        /// <summary>
        /// MEJORA #8: Salda un documento (asocia un pago con un documento a pagar).
        /// </summary>
        [HttpPost("saldar")]
        public IActionResult Saldar([FromBody] SaldarRequest request)
        {
            try
            {
                if (string.IsNullOrEmpty(request.RutaEmpresa))
                ...
                var resultado = _sdkService.SaldarDocumento(
                    request.RutaEmpresa,
                    request.CodConceptoPagar, request.SeriePagar, request.FolioPagar,
                    request.CodConceptoPago, request.SeriePago, request.FolioPago,
                    request.Importe, request.IdMoneda <= 0 ? 1 : request.IdMoneda,
                    request.Fecha ?? DateTime.Now.ToString("MM/dd/yyyy"));

                return resultado.exito
                    ? Ok(new { success = true, message = resultado.mensaje })
                    : BadRequest(new { success = false, message = resultado.mensaje });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error al saldar documento");
                return StatusCode(500, new { success = false, message = ex.Message });
            }
        }

        /// <summary>
        /// MEJORA #9: Obtiene el UUID (folio fiscal) de un CFDI timbrado.
        /// </summary>
        [HttpGet("uuid")]
        public IActionResult ObtenerUuid([FromQuery] string rutaEmpresa, [FromQuery] string codigoConcepto, [FromQuery] string serie, [FromQuery] double folio)
        {
            try
            {
                if (string.IsNullOrEmpty(rutaEmpresa))
                    return BadRequest(new { success = false, message = "rutaEmpresa es requerida" });
                var resultado = _sdkService.ObtenerUuid(rutaEmpresa, codigoConcepto, serie ?? "", folio);
                return resultado.exito
                    ? Ok(new { success = true, uuid = resultado.uuid })
                    : BadRequest(new { success = false, message = resultado.mensaje });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error al obtener UUID");
                return StatusCode(500, new { success = false, message = ex.Message });
            }
        }

        /// <summary>
        /// MEJORA #9: Lee un dato específico del CFDI timbrado (UUID, fecha, sello, régimen, etc.).
        /// dato: 1=SerieCertEmisor, 2=UUID, 3=SerieCertSAT, 4=FechaHoraCert, 5=SelloCFDI,
        ///       6=SelloSAT, 7=CadenaOriginal, 8=MetodoPago, 9=LugarExpedicion, 10=RegimenFiscal.
        /// </summary>
        [HttpPost("datos-cfdi")]
        public IActionResult ObtenerDatoCfdi([FromBody] ObtenerDatoCfdiRequest request)
        {
            try
            {
                if (string.IsNullOrEmpty(request.RutaEmpresa))
                ...
                var resultado = _sdkService.ObtenerDatoCfdi(
                    request.RutaEmpresa, request.Password ?? "",
                    request.CodigoConcepto, request.Serie ?? "", request.Folio, request.Dato);

                return resultado.exito
                    ? Ok(new { success = true, dato = request.Dato, valor = resultado.valor })
                    : BadRequest(new { success = false, message = resultado.mensaje });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error al obtener dato CFDI");
                return StatusCode(500, new { success = false, message = ex.Message });
            }
        }
    ...
    public class SaldarRequest
    {
        public string RutaEmpresa { get; set; } = "";
        public string CodConceptoPagar { get; set; } = "";
        public string SeriePagar { get; set; } = "";
        public double FolioPagar { get; set; }
        public string CodConceptoPago { get; set; } = "";
    ...
    public class ObtenerDatoCfdiRequest
    {
        public string RutaEmpresa { get; set; } = "";
        public string CodigoConcepto { get; set; } = "";
        public string Serie { get; set; } = "";
