    ...
    public class IntegracionController : ControllerBase
    {
        ...
        public IActionResult FlujoCompleto([FromBody] IntegracionRequest request)
        {
            try
            {
                ...
                var productos = new List<(string codigo, double cantidad, double precio, string nombre, string unidadMedida, string claveSAT)>
                {
                    (request.Producto.Codigo, request.Factura.Cantidad, request.Producto.Precio, request.Producto.Nombre ?? "", request.Producto.UnidadMedida ?? "", request.Producto.ClaveSAT ?? "")
                };
